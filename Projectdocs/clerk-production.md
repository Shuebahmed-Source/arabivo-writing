...
**This is not caused by Stripe or a subscription paywall.** Subscription code does not protect `/sign-in` or `/sign-up` (only `/dashboard` and **`/lessons`** use **`auth.protect()`** in **`proxy.ts`** on **production** — Preview/local may bypass **`protect`** for learn routes; see **`features.md`**). **`/`** and **`/try`** are also public.
